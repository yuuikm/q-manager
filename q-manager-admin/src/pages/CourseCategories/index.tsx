...
const CourseCategories = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const baseUrl = 'http://localhost:8000/api/admin/course-categories';
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const response = await fetch(`http://localhost:8000/api/admin/course-categories/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
    ...
    <div className="space-y-6">
      <div className="admin-card">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-2xl font-semibold text-gray-800">Управление категориями курсов</h2>
            <p className="text-gray-600 mt-1">Создавайте и редактируйте категории для курсов</p>
          </div>
          <div className="flex space-x-3">
            <button
              onClick={() => navigate(LINKS.coursesLink)}
        ...
        {categories.length === 0 && (
          <div className="text-center py-8">
            <p className="text-gray-600 text-lg mb-4">Категории курсов не найдены</p>
